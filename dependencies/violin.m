...
for i=i:size(Y,2)
    if isempty(lc) == 1
        ...
        if setX == 0
            h(i)=patch('XData',[F(:,i)+i;flipud(i-F(:,i))],'YData',...
                [U(:,i);flipud(U(:,i))],'facecolor',fc(i,:),'FaceAlpha',alp,'EdgeColor',lc(i,:));
        else
            h(i)=patch('XData',[F(:,i)+x(i);flipud(x(i)-F(:,i))],...
                'YData',[U(:,i);flipud(U(:,i))],'facecolor',fc(i,:),'FaceAlpha',alp,'EdgeColor',lc(i,:));
        end
    end
    hold on
    if setX == 0
